...
def process_frame(frame):
    ...
    `frame` is the raw, un-mirrored camera capture (the frontend's live
    preview is mirrored via CSS for the user's benefit while aligning the
    cube, but it captures the raw frame as-is - see captureImage() in
    script.js). Detection runs on this true orientation so the resulting
    sticker order/colors reflect the cube's actual physical layout. The
    annotated image returned for display is therefore also un-mirrored; it's
    just a brief "here's what we detected" confirmation shot rather than
    something the user actively aligns against, so not mirror-matching the
    live preview is a minor, acceptable cosmetic difference.
    """
    detected_colors = []
